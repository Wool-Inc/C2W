...
/**
 * Central event hub. Owns every subscription to Bukkit events, the internal
 * event bus, and the ProtocolLib packet listeners. Other managers and game
...
public class EventManager implements AutoCloseable {
    ...
    /**
     * Production constructor. Wires the ProtocolLib packet manager and registers
     * the Bukkit event listeners. Requires a live server.
     */
    ...
    /**
     * Headless constructor for tests. Skips ProtocolLib and Bukkit listener
     * registration so the event hub can be used without a running server. The
    ...
    public void close() {
        ...
        if (this.plugin == null || this.protocolManager == null) return;
